  ...
  return (
    <div>
                  ...
                  <td className="w-[190px] max-w-[190px] px-2 py-2">
                    ...
                    {r.marginPercent != null && (
                      <p
                        ...
                        style={{ color: r.marginPercent < 0 ? "#e34948" : "var(--ink-faint)" }}
                      >
                        {r.marginPercent < 0 ? "Sold at a loss — " : ""}
                  ...
                  <td className="px-2 py-2">
                    <div className="flex items-center gap-2">
                      <span className="h-3 min-w-0 flex-1 overflow-hidden rounded-full bg-[var(--mist)]">
                        {/* A negative margin (cost above sale price) used to
                            collapse to a 0%-width bar here, which rendered
                            identically to a real 0% margin — silently hiding
                            exactly the products a sales manager most needs to
                            see. A loss now draws as a red bar the length of
                            the shortfall, growing right-to-left off the sale
                            price's own position. */}
                        {r.costPerUnitSold != null && r.costPerUnitSold > r.saleRate ? (
                          <span className="flex h-full">
                            <span
                              className="h-full rounded-l-full bg-[#94a3b8]"
                              style={{ width: `${(r.saleRate / maxPrice) * 100}%` }}
                            />
                            <span
                              className="h-full bg-[#e34948]"
                              style={{ width: `${((r.costPerUnitSold - r.saleRate) / maxPrice) * 100}%` }}
                            />
                          </span>
                        ) : (
                          <span className="flex h-full">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-4 border-t border-[var(--line)] pt-3">
        ...
        <Key color="#e34948" label="Loss (cost above sale price)" />
